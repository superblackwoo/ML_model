...
print(data_tr.iloc[:, 40:].corr())  # 上面手动分析太累，还是利用corr方法自动分析  TODO:看这个.corr()的原理
...
class COVID19Dataset(Dataset):
    ...
    def __init__(self, path, mu, std, mode='train', target_only=False):
        ...
        with open(path, 'r') as fp:
            ...
            data = np.array(data[1:])[:, 1:].astype(float)  # 数据数组化，并且删除掉第一列
        ...
        if self.mode == 'test':
            ...
            indices_tr, indices_dev = train_test_split([i for i in range(data.shape[0])], test_size=0.3,
                                                       random_state=0)  # TODO:test_size:划分验证集的比例
            if self.mode == 'train':
                indices = indices_tr  # """下标索引"""
            elif self.mode == 'dev':
                indices = indices_dev
        ...
        if self.mode == "train":  # 如果是训练集，均值和方差用自己数据
            ...
            self.mu = mu  # 测试集和开发集，传进来的均值和方差是来自训练集保存，如何保存均值和方差，看数据dataload部分
            self.std = std
...
def prep_dataloader(path, mode, batchsize, n_jobs=0, target_only=False, mu=None,
    ...
    if mode == 'train':  # 如果是训练集，把训练集上均值和方差保存下来
        mu = dataset.mu
        std = dataset.std
    dataloder = DataLoader(dataset, batchsize, shuffle=(mode == 'train'), drop_last=False, num_workers=n_jobs,
                           pin_memory=True)
    return dataloder, mu, std
...
def dev(dv_set, model, device):
    ...
    for x, y in dv_set:  # iterate through the dataloader
        ...
        with torch.no_grad():  # disable gradient calculation
            pred = model(x)  # forward pass (compute output)
            mse_loss = model.cal_loss(pred, y)  # compute loss
        total_loss += mse_loss.detach().cpu().item() * len(x)  # accumulate loss  TODO detach：阻断反向传播. len(x):验证集的大小=行数
...
def test(tt_set, model, device):
    model.eval()  # set model to evalutation mode
    preds = []
    for x in tt_set:  # TODO: 每个For循环一个Batch
        x = x.to(device)
        with torch.no_grad():
            pred = model(x)
...
# TOD: How to tune these hyper-parameters to improve your model's performance? 这里超参数没怎么调，已经最优的了
config = {
...
dv_set, mu_none, std_none = prep_dataloader(tr_path, 'dev', config['batch_size'], target_only=target_only, mu=tr_mu,
                                            std=tr_std)
tt_set, mu_none, std_none = prep_dataloader(tt_path, 'test', config['batch_size'], target_only=target_only, mu=tr_mu,
                                            std=tr_std)

model = NeuralNet(tr_set.dataset.dim).to(device)  # TODO:tr_set.dataset.dim 数据feature的维度
model_loss, model_loss_record = train(tr_set, dv_set, model, config, device)

plot_learning_curve(model_loss_record, title='deep model')

print(dev(dv_set, model, device))  # 验证集损失
